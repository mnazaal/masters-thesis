...
from mincontexts import minimal_context_dags1,binary_minimal_contexts1
...
def colour_cstree(c, 
    ...
    while level<levels+1:
        ...
        stages_l = {color:nodes for color,nodes in stages.items() if len(nodes[0])==level}
        ...
        for n1,n2 in combinations(nodes_l,2):
            ...
            if colour_scheme.values():
                ...
                print("coloured nodes are", len(coloured))
            ...
            # Case (b,c,d,e) : Other than above
                ...
                #print(n1,n2, ordering[level-1], var, level)
...
def cstree_pc1(dataset, 
    ...
    for mec_dag_num, mec_dag in enumerate(dags_bn):
        ...
        for order_num, ordering in enumerate(orderings):
            ...
            if not use_dag:
                mec_dag = None
            tree,stages,cs = dag_to_cstree1(val_dict, ordering, mec_dag)
            ...
            t1=time.time()
            tree, stages, cs = colour_cstree(tree, ordering, dataset, stages, cs, test=test)
            print("Time taken to learn CSTree from further tests {}, ordering number {} is {}".format(mec_dag_num, 
                                                                                                      order_num, 
                                                                                                      time.time()-t1))
            ...
            #print(csi_rels)
            all_mc_graphs = minimal_context_dags1(ordering, csi_rels, val_dict)
            mecdag_count=0
